  ...
  public static class WebSocketTransciever
  {
      ...
      new Response
      {
        ...
        Headers =
        {
          ("Sec-Websocket-Accept", Convert.ToBase64String(new SHA1CryptoServiceProvider().ComputeHash(
            Encoding.ASCII.GetBytes(request.Headers.Find(h => h.Item1 == "sec-websocket-key").Item1 +
    ...
    public static byte[] ReadMessage(Stream stream, out byte opcode)
    {
      ...
      do
      {
        ...
        if (opcode == 255) opcode = (byte) (firstByte & 0xf);
    ...
    private static byte[] ReadFrame(Stream stream)
    {
      ...
      {
        length = 0;
        for (var i = 0; i < 8; i++) length = (length << 8) + (ulong) stream.ReadByte();
      }
      ...
      for (ulong i = 0; i < length; i++) content[i] ^= mask[i % 4];
