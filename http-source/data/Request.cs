...
namespace com.tempestasludi.c.http_source.data
{
  public class Request : Message
  {
        ...
        {
          {
            "cookie",
            (request, cookies) =>
              request.Cookies = cookies.Split(";".ToCharArray()).Select(s => Cookie.Read(s.Trim())).ToList()
          },
    ...
    protected override List<(string, string)> GetHeaders()
    {
      ...
      if (Cookies.Count > 0)
        headers.Add(("Cookie", Cookies.Select(cookie => cookie.ToString()).Aggregate((a, b) => $"{a}; {b}")));
